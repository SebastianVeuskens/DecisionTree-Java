...
//instances
public class DecisionTreeExtended {

  public Node root;
  public Object[][] data;

  public DecisionTreeExtended() {
    root = new Node();
  }

  /**
   *Apply the DecisionTreeExtended algorithm on the dataset with default seperator ','
   ...
   */
  public static DecisionTreeExtended decTreeExt(Dataset data, Integer label, Integer[] attributes) {
    DecisionTreeExtended model = new DecisionTreeExtended();
  ...
  Integer[] attributes) {
     ...
     for (int i = 1; i <= n; i++) {
       ...
       DecisionTreeExtended croValTree = train(trainInstances, data, label, attributes);
       result[i - 1] = croValTree.test(testInstances, label);
     }
     return result;
   }

  public static DecisionTreeExtended train(Integer[] instances, Dataset data, Integer label,
  Integer[] attributes) {
    DecisionTreeExtended model = new DecisionTreeExtended();
    label = model.setData(data, attributes, label);
    model.root.addData(model.findFrequLabel(instances, label));
  ...
  HashMap<Object,Double>[] frequencies, Integer label, Double infoD) {
    for (int att = 0; att < gainRatio.length; att++) {
      if (att != label) {
        ...
        if (splitInfo != 0.0) {
          gainRatio[att] = (infoD - info[att]) / Math.pow(2, splitInfo);
        } else {
          gainRatio[att] = 0.0;
        }
      }
